...
class batch_norm(object):
    ...
    def __call__(self, x, is_training=True):
        return tf.contrib.layers.batch_norm(x, decay=self.momentum, updates_collections=None,
                                            epsilon=self.epsilon, scale=True, scope=self.name, is_training=is_training)
...
#           name="conv2d"):
#    with tf1.variable_scope(name):
#        w = tf1.get_variable('w', [k_h, k_w, input_.get_shape()[-1], output_dim],
#                            initializer=tf.truncated_normal_initializer(stddev=stddev))
#        conv = tf.nn.conv2d(input_, w, strides=[1, d_h, d_w, 1], padding='SAME')
#
#        biases = tf1.get_variable('biases', [output_dim], initializer=tf.constant_initializer(0.0))
...
def deconv2d(input_, output_shape, padding='SAME',
    ...
    with tf1.variable_scope(name):
        if weight_params is not None:
            ...
            biases = tf1.get_variable('biases', [output_shape[-1]], initializer=tf.constant_initializer(0.0))
...
def linear(input_, output_size, scope=None, stddev=0.02, bias_start=0.0, with_w=False):
    ...
    with tf1.variable_scope(scope or "Linear"):
        matrix = tf1.get_variable("Matrix", [shape[1], output_size], tf.float32,
